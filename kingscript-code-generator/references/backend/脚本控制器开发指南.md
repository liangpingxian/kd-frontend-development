...
// 返回列表数据（必须用 ArrayList，禁止直接用 JS 数组）
// JS 原生 [] 会被序列化成 {} 空对象，必须转为 Java 集合类型
const items = new ArrayList();
items.add(new HashMap() /* put fields */);
response.ok({ items: items });
```

> **序列化约束**：`response.ok` 的入参中，所有 JS 原生数据结构必须转为 Java 集合类型，否则序列化结果异常。
> 详见 `controller-safe-template.md` 中的 `toJavaSafe` 工具函数。
>
> | JS 类型 | Java 类型 |
> |---------|----------|
> | `[]` (Array) | `ArrayList` |
...
class ResponseExample {
  ...
  errorExample(request: any, response: any) {
    try {
      ...
      response.throwException('操作失败: ' + e, 500, 'BUSINESS_ERROR');
    }
  }
...
| `qfilters` | array | 过滤条件数组，无条件传 `[]` |
| `orderBy` | string | 排序字段，无排序传 `''` |

> **禁止传入第 5 个参数**（如 limit），否则返回 HTTP 500。

### 字段名规则

- **单头字段**：直接写字段名，如 `field1,amountfield,bizdate`
- **分录字段**：必须带分录标识前缀，如 `entryentity.kdtest_combofield`
- 前缀取决于分录标识，最常见的默认分录标识为 `entryentity`，多分录实体可能为 `entryentity1` 等
- 字段名必须与实体元数据定义严格匹配，拼写错误会直接导致 500 空响应体

### 异常处理约定

```ts
// 禁止
catch (e) { response.throwException(e.getMessage(), 500, 'ERROR'); }  // .getMessage() 不可靠

// 正确
try {
  // ...
} catch (e) {
  response.throwException('操作失败: ' + e, 500, 'ERROR');  // 用 '' + e
}
```

> Java 异常对象的 `.message` / `.getMessage()` 在 KS 运行时不稳定，可能二次抛异常。统一用 `'' + e` 通过字符串拼接触发 `toString()`。
