...
```bash
node $SKILL_DIR/project-init.mjs --name <项目名> --framework react --language ts --app <应用编码>
```

- 脚本会自动检测并安装 kd CLI（无需手动安装）
- 自动完成工程初始化的所有步骤（框架选择、语言选择、app 输入）
- 自动执行 `npm install`
...
- 脚本启动后会自动检查环境是否已存在（通过 `kd env list`）
- 环境已存在时跳过创建，直接执行认证
- 环境不存在时执行完整流程：创建环境 → 认证
- 数据中心选择会自动选第一个
- 执行完毕后自动验证环境配置状态
...
- 先有环境 URL
- 再通过 setup-env.mjs 脚本自动完成认证
- 脚本会自动读取该环境可用的数据中心列表
- 数据中心由 setup-env.mjs 脚本自动选择
...
- 不要假设当前一定支持弹窗或表单式输入
- 在当前常规模式下，先让用户回填手工字段，数据中心由 setup-env.mjs 脚本自动选择
...
**格式二：有关联业务实体**
```
:::render:kdform {"title":"<masterLabel值>","url":"<环境URL>/?formId=<页面name>","metadata":"<环境URL>/kapi/v2/<ISV前缀>/mdl/meta-query/getEntityFields?formNumber=<实体formNumber>"}:::
```
...
| `metadata` | 条件 | 仅当页面关联了业务实体时包含。值为 `{环境URL}/kapi/v2/{ISV前缀}/mdl/meta-query/getEntityFields?formNumber={实体formNumber}` |

**URL 拼接规则：**
- 环境 URL：从 `kd env info` 或 `~/.kd/config.json` 中读取当前环境的 url
- formId：直接使用 `.page-meta.kwp` 中的 `<name>` 值（已包含 ISV 前缀），如 `kdtest_inv_dashboard`

> ⚠️ 这是一个**链接卡片**，不是总结。JSON 中仅包含上述字段（`title`、`url`、`metadata`（条件））。
...
- 链接卡片 JSON 仅含 `title`、`url`、`metadata`（条件）三个字段
- 所有值从实际文件和环境中读取，禁止猜测
