...
```html
<template>
  <sl-radio-group label="选择一个选项" name="option" value={value} onsl-change={handleChange}>
...
export default class RadioGroupLayoutDemo extends LightningElement {}
```
...
export default class RadioButtonLayoutDemo extends LightningElement {}
```
...
| 事件名称 | LWC 绑定 | 描述 |
| --- | --- | --- |
| sl-change | `onsl-change` | 选中值变化时触发 |
| sl-input | `onsl-input` | 用户输入时触发 |
| sl-invalid | `onsl-invalid` | 表单验证失败时触发 |
