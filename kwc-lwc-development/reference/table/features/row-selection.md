...
通过 `row-selection` 开启选择列，支持 `checkbox` 和 `radio`。
...
export default class TableRowSelection extends KingdeeElement {
  columns = [
...
## 注意事项
1. **rowKey 必须唯一**：`row-key` 指定的字段在数据中必须唯一，否则会导致行选择功能异常
