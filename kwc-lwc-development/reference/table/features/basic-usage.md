...
用于演示 `sl-table` 的最小可用配置：`row-key`、`columns`、`data-source`。
...
```html
<template>
  <sl-table kwc:external row-key="id" columns={columns} data-source={dataSource}></sl-table>
</template>
```
...
1. **rowKey 必须唯一**：`row-key` 指定的字段在数据中必须唯一，否则会导致行选择、行展开等功能异常
2. **columns 必须包含 dataIndex**：每列配置必须指定 `dataIndex`，且需与数据源中的字段名对应
3. **建议设置列宽**：为每列设置 `width` 可避免列宽分配不合理的问题，未设置宽度的列会自动分配剩余空间
4. **数据源更新**：在 LWC 中修改 `dataSource` 需要使用 `@track` 装饰器或重新赋值数组以触发重新渲染