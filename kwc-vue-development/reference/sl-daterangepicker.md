...
使用 `:value.prop` 绑定初始日期范围，Vue 中传入 `[Date | null, Date | null]` 元组。注意复杂类型（数组/对象）必须使用 `.prop` 修饰符。
...
使用 `:placeholder.prop` 绑定两个输入框的占位文本，传入 `[string, string]` 数组。
...
```html
<template>
  <sl-daterangepicker style="width: 400px" label="斜杠格式" format="YYYY/MM/DD"></sl-daterangepicker>
</template>
...
```html
<template>
  <sl-daterangepicker style="width: 460px" label="中文格式" format="YYYY年MM月DD日"></sl-daterangepicker>
</template>
...
```html
<template>
  <sl-daterangepicker style="width: 500px" label="自定义 datetime 格式" type="datetime" format="YYYY/MM/DD HH:mm"></sl-daterangepicker>
</template>
...
<template>
  <div style="display: flex; flex-direction: column; gap: 16px">
    <sl-daterangepicker style="width: 400px" label="默认 (YYYY-MM-DD)" :value.prop="dateValue"></sl-daterangepicker>
    <sl-daterangepicker style="width: 400px" label="斜杠 (YYYY/MM/DD)" format="YYYY/MM/DD" :value.prop="dateValue"></sl-daterangepicker>
    <sl-daterangepicker style="width: 460px" label="中文 (YYYY年MM月DD日)" format="YYYY年MM月DD日" :value.prop="dateValue"></sl-daterangepicker>
    <sl-daterangepicker style="width: 400px" label="欧洲 (DD.MM.YYYY)" format="DD.MM.YYYY" :value.prop="dateValue"></sl-daterangepicker>
  </div>
</template>
...
在 Vue 中通过 `@sl-change`、`@sl-input` 等语法监听 Shoelace 自定义事件。
...
<script setup>
  ...
  function handleChange(event) {
    changeValue.value = formatRange(event.target.value);
    console.log('sl-change:', event.target.value);
  }

  function handleInput(event) {
    inputValue.value = formatRange(event.target.value);
  }
</script>
```
...
通过模板 `ref` 动态读取和设置日期范围选择器的值。`value` 属性为 `[Date | null, Date | null]` 元组。

```html
<template>
  <div style="display: flex; flex-direction: column; gap: 12px; width: 400px">
    <div style="display: flex; gap: 8px">
      <sl-button size="small" @click="handleThisMonth">本月</sl-button>
...
<script setup>
  ...
  function handleThisMonth() {
    ...
    daterangeRef.value.value = [start, end];
  }
  ...
  function handleClear() {
    daterangeRef.value.value = [null, null];
  }
</script>
```
...
通过模板 `ref` 调用 `setCustomValidity()` 方法设置自定义校验消息，实现业务逻辑校验。
...
<script setup>
  ...
  function handleChange() {
    daterangeRef.value?.setCustomValidity('');
  }

  function handleValidate() {
    const picker = daterangeRef.value;
    if (!picker) return;
...
在 Vue 中通过 `@事件名` 语法监听以下自定义事件。

| 事件名称   | Vue 绑定语法 | 描述                                                             |
| ---------- | ------------ | ---------------------------------------------------------------- |
| sl-change  | @sl-change   | 当用户确认值的修改时触发（选择完成关闭面板、清除值时）。         |
| sl-input   | @sl-input    | 当值实时变化时触发（用户在面板中点选日期时、在输入框中输入时）。 |
| sl-focus   | @sl-focus    | 当控件获得焦点时触发。                                           |
| sl-blur    | @sl-blur     | 当控件失去焦点时触发。                                           |
| sl-clear   | @sl-clear    | 当清除按钮被激活时触发。                                         |
| sl-invalid | @sl-invalid  | 当表单控件校验失败时触发。                                       |
...
Vue 中绑定复杂类型（数组/对象）必须使用 `.prop` 修饰符，`value` 使用 `[Date | null, Date | null]` 元组：
...
<script setup>
  ...
  // 通过模板 ref 读取值
  const daterangeRef = ref(null);
  const [start, end] = daterangeRef.value?.value ?? [null, null];
  // start: Date | null, end: Date | null

  // 也可以通过便捷属性读取
  const startDate = daterangeRef.value?.startDate; // Date | null
  const endDate = daterangeRef.value?.endDate; // Date | null
</script>
```
...
Vue 默认将绑定值作为 HTML attribute 设置，而 `value`（数组）和 `placeholder`（数组）等复杂类型必须作为 property 传递：
...
<script setup>
  ...
  // 读取值
  const [start, end] = daterangeRef.value.value;

  // 设置值
  daterangeRef.value.value = [new Date(2025, 0, 1), new Date(2025, 5, 30)];

  // 调用方法
  daterangeRef.value.setCustomValidity('自定义错误');
  daterangeRef.value.reportValidity();
</script>
```
...
A: 确保使用了 `.prop` 修饰符绑定数组值：`:value.prop="[new Date(...), new Date(...)]"`。如果直接使用 `:value`，Vue 会将数组序列化为字符串作为 attribute 设置，导致组件无法正确解析。
...
<script setup>
  ...
  // 或使用便捷属性
  const startDate = daterangeRef.value?.startDate;
  const endDate = daterangeRef.value?.endDate;
</script>
```
